...
public class GenericDownloader<C> {
  ...
  private final CheckpointHeaderTaskProvider checkpointHeaderTaskProvider;
  ...
  public GenericDownloader(
      ...
      final CheckpointHeaderTaskProvider checkpointHeaderTaskProvider,
    ...
    this.checkpointHeaderTaskProvider = checkpointHeaderTaskProvider;
  ...
  private CompletableFuture<?> pullCheckpointHeaders(final SyncTarget syncTarget) {
    ...
    return checkpointHeaderTaskProvider
        .checkpointHeadersTask(lastHeader, syncTarget)
        .run()
        .handle(
            (r, t) -> {
  ...
  public interface CheckpointHeaderTaskProvider {
    EthTask<PeerTaskResult<List<BlockHeader>>> checkpointHeadersTask(
        BlockHeader lastCheckpointHeader, SyncTarget syncTarget);
  }

  public interface SyncTargetManager {
    CompletableFuture<SyncTarget> findSyncTarget();

    boolean isSyncTargetDisconnected();

    void clearSyncTarget(final SyncTarget syncTarget);

    boolean shouldSwitchSyncTarget(SyncTarget currentTarget);
  }
